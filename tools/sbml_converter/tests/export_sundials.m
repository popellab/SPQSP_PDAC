% Run MATLAB sundials solver and export with proper column names
if ~exist('output_csv', 'var')
    error('Set output_csv');
end
...
for i = 1:n_cols
    ...
    if bare_to_comp.isKey(n)
        ...
        if iscell(mapping)
            if seen_count.isKey(n); seen_count(n)=seen_count(n)+1; else; seen_count(n)=1; end
            idx = seen_count(n);
            if idx<=length(mapping); col_names{i}=mapping{idx}; else; col_names{i}=[n '_' num2str(idx)]; end
        else
            if seen_count.isKey(n); col_names{i}=n; else; seen_count(n)=1; col_names{i}=mapping; end
        end
    else
        col_names{i} = n;
    end
...
for i = 1:n_cols
    n = col_names{i};
    if final_seen.isKey(n); final_seen(n)=final_seen(n)+1; col_names{i}=[n '_dup' num2str(final_seen(n))]; else; final_seen(n)=1; end
end

T = array2table([t, data], 'VariableNames', [{'Time'}, col_names]);
writetable(T, output_csv);
fprintf('Wrote %d points, %d cols to %s\n', size(data,1), n_cols, output_csv);
